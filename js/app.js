...
function con($scope, calculator, buyCalculation, $http, indeedTax, businessTax, sellCalculation){
        ...
        $http.get("http://app.dooioo.com:8080/app/caleVersion").success(function(data) {
            ...
            if ($scope.newpolicy) {
                for(var i=0; i < $scope.newpolicy.length; i++){
                    if ($scope.newpolicy[i].version == 1.1) {
                        newNum = i;
                    }
                }                
    ...
    $scope.showSellResult = function() {
        ...
        $scope.filter2.lastPrice1 = $scope.filter2.lastPrice1 ? parseFloat($scope.filter2.lastPrice1) : 0.00;
        ...
        if (!$scope.calType) {
            ...
            if (!$scope.filter2.lastPrice1 || !$scope.filter2.area) {
                essentialFlag = true;
            }
        }
        ...
        if (essentialFlag) {
            ...
            if (!$scope.calType) {
                ...
                $scope.filter2.businessTaxResult = sellCalculation.businessTaxCal($scope.filter2.contractPrice, $scope.fee.purchasePrice, $scope.bt); // 营业税
                if (!$scope.filter2.isDeedTax) {
                    var primeCost = $scope.fee.purchasePrice + $scope.fee.interest + $scope.fee.decorationCost + $scope.fee.buyDeedTax + $scope.filter2.businessTaxResult;            
                }else if ($scope.filter2.isDeedTax) {
                    var primeCost = $scope.fee.purchasePrice + $scope.fee.interest + $scope.fee.decorationCost + $scope.filter2.businessTaxResult;            
                }
                ...
                $scope.filter2.contractPrice = sellCalculation.contractPriceCal($scope.filter2.native, $scope.bt, $scope.it, $scope.fee, $scope.filter2.tradingFee, $scope.filter2.lastPrice1);
        ...
        // 到手价
        if (!$scope.calType) {
            $scope.filter2.lastPrice2 = $scope.filter2.contractPrice - $scope.filter2.totalTax;
        }

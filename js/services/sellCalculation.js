...
//     var self = this;
    ...
    sellCalculation.businessTaxCal = function(contractPrice, purchasePrice, bt) {
        ...
        if (bt.totalProportion) {
            ...
            tax = (contractPrice - purchasePrice) * bt.diffProportion;
        }
        return tax;
    }
    ...
    sellCalculation.contractPriceCal = function(native, bt, it, fee, tradingFee, lastPrice) {
        ...
        var temp1 = 1 - it.totalProportion - it.profitProportion - calculator.commissionFeeCal() - bt.totalProportion - bt.diffProportion + bt.totalProportion*it.profitProportion + bt.diffProportion*it.profitProportion ;
        // temp2 = 购入价 * 差额比例*（1 - 利润比例） + (购入价 + 装修 + 利润 + 购房契税 ,不包括营业税的购房成本) * 个税利润比例 - 交易手续费 
        var temp2 = fee.purchasePrice * bt.diffProportion*(1-it.profitProportion) + primeCost * it.profitProportion - tradingFee;
        // 合同价(有公证费/无公证费)
        if (!native) {
